...
module.exports = (service, endpoint) => {
  ...
  //added by hafsa
  service.get('/users/mobile-users', endpoint(({FieldKeys})=>
  FieldKeys.getAll()));
...
service.post('/users/mobile-users', endpoint(({ FieldKeys }, { auth, body }) => {
  return (async () => {
    // Generate the password
    const password = crypto.randomBytes(8).toString('hex');
    // Retrieve environment variables
    const env = config.get('default.env');
    // Create mail transport options
    const mailOptions = mergeRight(config.get('default.email'), { env });
    // Create mailer instance
    const mail = mailer(mailOptions);
    // Send the mail
    ...
    console.log('backend fk', fk);
    return FieldKeys.create2(fk);
