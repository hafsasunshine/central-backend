...
//added by hafsa
const create2 = (fk) => ({ Actors, Sessions}) => Actors.createSubtype(fk)
  .then((created) => Sessions.create(created.actor, new Date('9999-12-31T23:59:59z'))
        .then((session) => created.withAux('session', session)));
...
const getAll = (options = QueryOptions.none) => ({all}) =>
  _get(all, options.withCondition());

module.exports = { create,create2, getAllForProject, getByProjectAndActorId, getAll };
